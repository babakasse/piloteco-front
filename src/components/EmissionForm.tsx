  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      const emissionsToSend = emissions
        .filter(em =>
          em.source && em.category && em.activityData && em.emissionFactor && em.scope && em.unit
        )
        .map(em => ({
          ...em,
      ...
      <CardContent>
        {assessmentLoading ? (
          ...
          <Paper elevation={2} sx={{ p: 2, mb: 3, background: '#f7f7f7' }}>
            <Typography variant="subtitle1" gutterBottom>Bilan sélectionné</Typography>
            <Typography><b>Nom :</b> {assessment.name}</Typography>
            <Typography><b>Année :</b> {assessment.year}</Typography>
            <Typography><b>Description :</b> {assessment.description || '-'}</Typography>
            {assessment.status && <Typography><b>Statut :</b> {assessment.status}</Typography>}
          </Paper>
        ) : (
          <Typography color="error">Aucun bilan trouvé</Typography>
        )}
        <Typography variant="h6" gutterBottom>Ajouter des émissions</Typography>
        <Box component="form" onSubmit={handleSubmit}>
          ...
          {emissions.map((em, idx) => (
            <Grid container spacing={2} key={idx} alignItems="center" sx={{ mb: 1 }}>
              ...
              <Grid item xs={12} md={2}>
                <TextField label="Source" select fullWidth value={em.source} onChange={e => handleEmissionChange(idx, 'source', e.target.value)} required>
              ...
              <Grid item xs={12} md={2}>
                <TextField label="Catégorie" select fullWidth value={em.category} onChange={e => handleEmissionChange(idx, 'category', e.target.value)} required>
              ...
              <Grid item xs={12} md={2}>
                <TextField label="Activité" type="number" fullWidth value={em.activityData} onChange={e => handleEmissionChange(idx, 'activityData', e.target.value)} required />
              </Grid>
              <Grid item xs={12} md={1}>
                <TextField label="Facteur" type="number" fullWidth value={em.emissionFactor} onChange={e => handleEmissionChange(idx, 'emissionFactor', e.target.value)} required />
              </Grid>
              <Grid item xs={12} md={1}>
                <TextField label="Unité" fullWidth value={em.unit} onChange={e => handleEmissionChange(idx, 'unit', e.target.value)} />
              </Grid>
              <Grid item xs={12} md={1}>
                <TextField label="Scope" select fullWidth value={em.scope} onChange={e => handleEmissionChange(idx, 'scope', Number(e.target.value))}>
                  {scopes.map(scope => (
          ...
          <Button variant="outlined" onClick={addEmission} sx={{ mt: 1 }}>Ajouter une émission</Button>
          {error && <Typography color="error" mt={2}>{error}</Typography>}
          {success && <Typography color="success.main" mt={2}>Émission(s) ajoutée(s) avec succès !</Typography>}
          <Box mt={2}>
            <Button type="submit" variant="contained" disabled={loading}>{loading ? 'Envoi...' : 'Ajouter les émissions'}</Button>
          </Box>
        </Box>
      </CardContent>
    </Card>
